...
for (outer_i in outer_folds) {
      for (inner_j in inner_folds) {
            ...
            X_train_pep <- training_partitions %>% 
                  pull(Peptide) %>% 
                  one_hot_encoding(peptide = .,
                                   amino = amino_acids) %>% 
                  array_reshape(., c(nrow(training_partitions), 9, 20, 1))
            
            X_train_CDR3b <- training_partitions %>% 
                  pull(CDR3b) %>% 
                  one_hot_encoding(peptide = .,
                                   amino = amino_acids_x) %>%
                  array_reshape(., c(nrow(training_partitions), max_CDR3b, 21, 1))
            ...
            X_val_pep <- validation_partition %>% 
                  ...
                  one_hot_encoding(peptide = .,
                                   amino = amino_acids) %>% 
                  array_reshape(., c(nrow(validation_partition), 9, 20, 1))
            
            X_val_CDR3b <- validation_partition %>% 
                  pull(CDR3b) %>% 
                  one_hot_encoding(peptide = .,
                                   amino = amino_acids_x) %>%
                  array_reshape(., c(nrow(validation_partition), max_CDR3b, 21, 1))
            ...
            X_test_pep <- test_partition %>%
                  ...
                  one_hot_encoding(peptide = .,
                                   amino = amino_acids) %>%
                  array_reshape(., c(nrow(test_partition), 9, 20, 1))
            
            X_test_CDR3b <- test_partition %>%
                  pull(CDR3b)%>% 
                  one_hot_encoding(peptide = .,
                                   amino = amino_acids_x) %>%
                  array_reshape(., c(nrow(test_partition), max_CDR3b, 21, 1))
...
X_test_final_pep <- data_A0201_test %>% 
      ...
      one_hot_encoding(peptide = ., 
                       amino = amino_acids) %>% 
      array_reshape(., c(nrow(data_A0201_test), 9, 20, 1))

X_test_final_CDR3b <- data_A0201_test %>%
      pull(CDR3b)%>% 
      one_hot_encoding(peptide = ., 
                       amino = amino_acids_x) %>%  
      array_reshape(., c(nrow(data_A0201_test), max_CDR3b, 21, 1))
...
data_A0201_onehot_mdl_preds_test <- data_A0201_test
for (i in 1:20) {
      
      # Load model
      mdl_file <- str_c("models/onehot/mdl_", i, ".hdf5")
      ...
      data_A0201_onehot_mdl_preds_test <- data_A0201_onehot_mdl_preds_test %>%
...
write_tsv(x = data_A0201_onehot_mdl_preds_test,
          file = "data/04_ii_data_A0201_onehot_mdl_preds_test_ADAM50k.tsv.gz")
